...
public class WebSearchDocumentRetriever implements DocumentRetriever {
  ...
  private static final String TAVILY_API_KEY = "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";
  private static final String TAVILY_BASE_URL = "https://api.tavily.com/search";
  private static final int DEFAULT_RESULT_LIMIT = 5;
  private final int resultLimit;
  private final RestClient restClient;

  WebSearchDocumentRetriever(RestClient.Builder clientBuilder, int resultLimit) {
    Assert.notNull(clientBuilder, "clientBuilder must not be null");
    String apiKey = TAVILY_API_KEY;
    Assert.hasText(apiKey, "apiKey must not be empty");
    this.restClient = clientBuilder.baseUrl(TAVILY_BASE_URL).defaultHeader(HttpHeaders.AUTHORIZATION, "Bearer "+ apiKey).build();
    if(resultLimit <= 0) {
